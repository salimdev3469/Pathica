...
export default function LiveTokenCounter({
  ...
  return (
    <div className="flex min-w-0 items-center gap-1.5 sm:gap-2 shrink-0">
      <div className="inline-flex h-8 sm:h-9 items-center gap-1.5 sm:gap-2 rounded-full border border-gray-200 bg-white px-2 sm:px-3 text-[#111827] shadow-sm">
        <Coins className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-blue-600" />
        <span className="hidden sm:inline text-xs font-semibold text-gray-600">{'Credits'}</span>
        <span className="text-xs sm:text-sm font-bold tabular-nums text-[#111827]">
      ...
      {freeExports > 0 && (
        <div className="inline-flex h-8 sm:h-9 items-center gap-1.5 sm:gap-2 rounded-full border border-gray-200 bg-white px-2 sm:px-3 text-[#111827] shadow-sm">
          ...
          <span className="hidden sm:inline text-xs font-semibold text-gray-600">{'Exports'}</span>
          <span className="text-xs sm:text-sm font-bold tabular-nums text-[#111827]">
            {isLoading ? '-' : formatter.format(freeExports)}
          </span>
        </div>
      )}
      <div className="inline-flex h-8 sm:h-9 cursor-pointer items-center justify-center rounded-full border border-blue-200 bg-blue-50 px-2.5 sm:px-4 text-[11px] sm:text-xs font-bold text-blue-600 transition hover:bg-blue-100">
        <Plus className="mr-1 sm:mr-1.5 h-3 w-3 sm:h-3.5 sm:w-3.5" />
        {'Upgrade'}
      </div>
      {error ? <p className="w-full text-[10px] sm:text-xs text-red-600 font-semibold">{error}</p> : null}
    </div>
  );
}
